...
- **#4 Confidence calibration (ECE)** — `confidence_ratings.post` × acerto 1ª. Labels: 🎯 calibrated / 🔥 over-confident / 🧊 under-confident. Ref: Guo et al. 2017 (ICML, *On Calibration of Modern Neural Networks*) para fórmula ECE com binning equidistante; Hacker, Bol & Keener 2008 (cap. *Metacognition in Education*) como background metacognitivo.
- **#2 Response-time phenotyping** — diff de `answered_at` (janela 10s–15min), mediana por aluno vs. turma. 4 quadrantes (`fast/slow × correct/incorrect`). Ref: Wise & Kong 2005 (rapid-guessing detection, *Educ. Assessment*); Van der Linden 2007 como pano de fundo de modelagem hierárquica de RT.
...
- **#8 Cohort trajectory drift** — score composto por aula → percentil turma → 9 pontos → drift (queda ≥20 decis OU z-score rolling-3 < −1.5). Sparkline no modal. Flag 🔻 e sort `drift-desc`. Inspirado em literatura EDM (Educational Data Mining) sobre trajetórias de risco; thresholds calibrados internamente para o tamanho da turma MPE (n=50).
- **#10 Trend per phase (OLS)** — slope (regressão linear ordinária) de % acerto através das 4 fases (pre_review → embedded → post → graded_exercise) por aula. **Não** é "learning velocity" no sentido power-law (Newell & Rosenbloom 1981); é só descritor linear de melhoria entre fases. Pode ser negativo se acerto cai do pré-review para o pós (sinal de calibre mal ajustado).
- **#3 Heurística de comportamento (proxy SRL)** — 3 features (attempts/cp, revisit_ratio, read_before_try) + regras de threshold interpretáveis → labels `strategic/reactive/skimmer/avoidant/mixed`. **Não** é uma operacionalização canônica do framework SRL de Zimmerman ou Pintrich; é um proxy comportamental construído internamente para o contexto MPE. Thresholds documentados em comments do código + tunáveis no admin (Tab Psico-cognitivo).
