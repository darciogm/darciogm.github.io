...
**M1.** ✅ **RESOLVIDO em 2026-05-08** (commit `<commit-pending>`). Decisão de design: alterar dotações **na pré-aula** (EP-1 e EP-2), preservando a estrutura conceitual paralela mas calibração numérica diferente. EP-1: `ω^A = (50, 110)`, `ω^B = (110, 50)` (vs. Ex 1 com `(60, 100)/(100, 60)`); agregado segue livre de risco em (160, 160) e portfolio replicador agora resolve `Aθ^A = (30, -30)` em vez de `(20, -20)`. EP-2: `ω^A = (100, 60, 20)`, `ω^B = (20, 60, 100)` (vs. Ex 2 com `(90, 60, 30)/(30, 60, 90)`); agregado livre de risco em (120, 120, 120) preservado e o vetor inviável agora é `(-40, 0, 40)` em vez de `(-30, 0, 30)`. Adicionado nota explícita "parâmetros distintos dos do Ex 1/Ex 2 — treino formativo, não preview literal". Verificação: `grep` confirma EP-1=`(50, 110)`, Ex 1=`(60, 100)`; EP-2=`(100, 60, 20)`, Ex 2=`(90, 60, 30)`. **Anti-padrão CLAUDE.md eliminado.**
...
**m4.** ✅ **RESOLVIDO em 2026-05-08** (commit `<commit-pending>`). Word swap aplicado em duas frentes em paralelo: `aula_6/exercicios-avaliativos.md:129` e `platform/aula-06-exerc.html:179`. Texto novo: "payoff space \(\mathcal{M}\) é subespaço estrito (dimensão menor que \(|S|\)) de \(\mathbb{R}^S\)". Verificação: `grep "subespaço próprio"` nos dois arquivos retorna 0 ocorrências; `grep "subespaço estrito"` retorna ocorrências paralelas em MD e HTML.
