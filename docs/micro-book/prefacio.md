...
Confessemos logo: o mundo não estava desesperadamente carente de mais um livro-texto de microeconomia. As estantes — físicas e digitais — já vergam sob o peso de Varian, Mas-Colell, Nicholson & Snyder, Pindyck & Rubinfeld, Jehle & Reny, e uma constelação de manuais que, juntos, contêm praticamente tudo que um ser humano precisa saber sobre a teoria dos preços. Se o leitor já possui um desses volumes, por que deveria abrir este?

A resposta curta: porque nenhum deles foi escrito *para você*.

"Você", neste caso, é um estudante brasileiro de graduação avançada ou pós-graduação que precisa dominar microeconomia com rigor matemático, mas que também quer entender *por que* a teoria importa — não como exercício intelectual abstrato, mas como ferramenta para compreender o país em que vive. Um estudante que se cansou de ler sobre "um consumidor no mercado de maçãs" e gostaria de saber o que a TMS tem a ver com a POF do IBGE, o que o teorema de Coase diz sobre o desmatamento na Amazônia, e por que a teoria dos jogos explica as decisões do COPOM.

A resposta longa é este livro.
...
Além disso, **Boxes Mundo** trazem casos internacionais emblemáticos — da Toyota à TSMC, do Concorde ao *surge pricing* da Uber — para que o leitor veja a microeconomia em ação nos quatro cantos do planeta.
...
3. **"Vem, ANPEC!"** — questões selecionadas de provas da ANPEC (Associação Nacional de Pós-Graduação em Economia), com gabaritos e justificativas detalhadas item a item. Para quem se prepara para o exame, essa seção é ouro puro.
...
O objetivo é duplo: mostrar ao aluno que a teoria tem consequências empíricas testáveis, e expô-lo desde cedo à fronteira da pesquisa — algo que muitos livros-texto relegam a notas de rodapé, se mencionam.
...
O pré-requisito formal é cálculo de uma e várias variáveis. O Capítulo 2 revisa todo o ferramental necessário, mas espera-se familiaridade prévia com derivadas, integrais e álgebra linear elementar.

---

## Como usar este livro

**Leitura linear**: os capítulos foram organizados numa sequência pedagógica deliberada — cada conceito se constrói sobre os anteriores. O leitor que seguir a ordem dos capítulos encontrará uma narrativa coerente, com referências cruzadas que criam uma rede de conexões.

**Leitura seletiva**: cada capítulo é razoavelmente autocontido, com referências explícitas aos pré-requisitos. Um leitor que domine a teoria do consumidor pode saltar diretamente para a teoria da firma ou para a teoria dos jogos, consultando capítulos anteriores quando necessário.

**Para a ANPEC**: as seções "Vem, ANPEC!" de cada capítulo formam, juntas, um banco de questões comentadas que pode ser percorrido independentemente.
...
Nenhum livro é uma ilha. Este projeto nasceu da convicção de que os estudantes brasileiros merecem um material didático que combine rigor internacional com relevância local — e foi tornado possível pela confluência de talento humano e inteligência artificial.

A Claude, da Anthropic, colaborou extensivamente na redação, na verificação de consistência, na criação de exercícios e na construção dos gráficos interativos. Mas a inteligência artificial, por mais capaz que seja, não substitui a curadoria, a visão pedagógica e o julgamento crítico do autor — ela os amplifica. Cada parágrafo, cada equação e cada exemplo foram revisados com olhos humanos, e a responsabilidade final por qualquer erro remanescente é inteiramente do autor.

Agradeço aos colegas que comentaram versões preliminares, aos alunos que serviram como cobaias involuntárias (e surpreendentemente pacientes) das primeiras versões dos exercícios, e ao leitor que chegou até aqui — sinal de que pelo menos a introdução cumpriu seu papel de despertar curiosidade.
