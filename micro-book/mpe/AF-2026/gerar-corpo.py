...
Uso: python3 gerar-corpo.py && latexmk -pdf af-2026.tex af-2026-solucao.tex
...
def tex_safe(s):
    """Sanitiza metadados (fonte_paralela) para texto LaTeX: unicode math -> macros."""
    rep = {"Δ": "$\\Delta$", "σ": "$\\sigma$", "π": "$\\pi$", "θ": "$\\theta$",
           "β": "$\\beta$", "η": "$\\eta$", "ε": "$\\varepsilon$", "ρ": "$\\rho$",
           "≥": "$\\ge$", "≤": "$\\le$", "→": "$\\to$", "⇒": "$\\Rightarrow$",
           "×": "$\\times$", "√": "raiz ", "&": "\\&", "%": "\\%", "#": "\\#", "_": "\\_"}
...
for b in data["blocos"]:
    ...
    for k, o in enumerate(c["objetivas"]):
        ...
        W("\\begin{solucao}")
        W(f"{{\\footnotesize\\textit{{Paralelo de: {tex_safe(o.get('fonte_paralela','---').split('.')[0])}.}}}}\\par\\vspace{{0.2em}}")
        W(o["solucao_latex"])
        W("\\end{solucao}")
        W("")
    ...
    for j, s in enumerate(ab["subitens"]):
        ...
        W("\\begin{solucao}")
        if j == 0:
            W(f"{{\\footnotesize\\textit{{Paralelo de: {tex_safe(ab.get('fonte_paralela','---').split('.')[0])}.}}}}\\par\\vspace{{0.2em}}")
        W(s["solucao_latex"])
        W("\\rubricalabel")
        W(s["rubrica_latex"])
        W("\\end{solucao}")
